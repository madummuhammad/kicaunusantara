...
Class AudioPostService
{
    ...
    {
        return $this->query()->wherePostLanguage(LocalizationHelper::getCurrentLocaleId());
    }
    ...
    public function dynamicQuery($page, $layout, $widgetName, $widgetData, $localeId)
    {
        ...
        if ($postType == 'audio') {
            $query1 = (clone $query);
        } elseif ($postType == 'audio_by_category') {
            $term = Term::category()->where('slug', $category)->first();
            $query1 = $term->posts()->audio()->where('post_language', $localeId);
        }
    ...
    public function save($request)
    {
        ...
        if ($request->has('post_source')) {
            if ($request->post_type == 'audio_file') {
                $fileName = 'audio-' . Str::random(10);
                $fileInfo = Filepond::field($request->post_source)
                            ->moveTo('audios/' . $fileName);
    ...
    public function modify($request, $id)
    {
        ...
        if ($request->has('post_source')) {
            if ($request->post_type == 'audio_file') {
                ...
                $this->deleteFile('audios', $post->post_source);
            } else {
                $data['post_source'] = $request->post_source;

                if ($post->post_type == 'audio _file') {
                    $this->deleteFile('audios    ', $post->post_source);
                }
            }
